<?php
  ...
  if(isset($_SESSION['key']))
  {
    ...
    {
      $eid=@$_GET['eid'];
      $result = mysqli_query($con,"SELECT * FROM quizs WHERE eid='$eid' ") or die('Error');
      ...
      $r3 = mysqli_query($con,"DELETE FROM quiz WHERE eid='$eid' ") or die('Error');
  ...
  if(isset($_SESSION['key']))
  {
    if(@$_GET['q']== 'addqns' && $_SESSION['key']=='suryapinky') 
    {
      ...
      for($i=1;$i<=$n;$i++)
      {
        $qid=uniqid();
        $qns=$_POST['qns'.$i];
        $q3=mysqli_query($con,"INSERT INTO quiz VALUES  ('$eid','$qid','$qns' , '$ch' , '$i')");
        $oaid=uniqid();
        $obid=uniqid();
        $ocid=uniqid();
        $odid=uniqid();
